...
export function KnockoutPage() {
  ...
  return (
    <div className="page" data-testid="knockout-page">
      <h1>{viewerMode ? 'Viewer — Knockout stage' : 'Knockout stage'}</h1>
      <p className="page-intro">
        {viewerMode ? (
          <>
            Everyone's knockout guesses, revealed at kickoff next to the actual result.
            Fixtures fill in as group standings settle and earlier rounds finish; until then
            they show their placeholder (e.g. <code>1A</code>, <code>W74</code>).
          </>
        ) : (
          <>
            Predict every knockout scoreline — bets lock at kickoff. Fixtures fill in as
            group standings settle and earlier rounds finish; until then they show their
            placeholder (e.g. <code>1A</code>, <code>W74</code>). You can still bet on a
            scoreline before the teams are known.
          </>
        )}
      </p>
