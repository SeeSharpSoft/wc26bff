...
src/
  ...
  storage/         # localStorage.ts (typed wrapper + keys + fallback); users.ts, bets.ts accessors
  domain/          # pure logic: users.ts, bets.ts (immutable get/set/clear over BetsByUser)
  context/         # *Context.ts (context+hook) + *Provider.tsx pairs: User…, Bets…
  components/      # Header, UserSwitcher, MatchCard, BetInput; Bracket/Leaderboard (Phase 5+)
  pages/           # GroupsPage, SchedulePage; KnockoutPage, ViewerPage (Phase 5+)
  hooks/           # useNow.ts (interval-refreshed clock for live locking)
  utils/           # time.ts (local-tz formatting), locking.ts; scoring/standings in Phase 4+
  types.ts         # domain model (single source of truth)
  App.tsx          # HashRouter + active-user bar; routes: / (Groups), /schedule
  main.tsx         # mounts <UserProvider><BetsProvider><App/></BetsProvider></UserProvider>
scripts/
  build-data.mjs   # source .txt -> src/data/generated.ts generator
tests/
  unit/            # Jest unit tests (data, users, storage, bets, locking, time)
  e2e/             # Playwright browser tests (groups, betting, users)
```
...
| `src/components/MatchCard.tsx` | One fixture: teams/flags, kickoff (local tz), result/LIVE, bet. |
| `src/components/BetInput.tsx` | Score-guess inputs; read-only 🔒 once locked. |
| `src/pages/GroupsPage.tsx` | 12 group sections, each with its 6 `MatchCard`s. |
